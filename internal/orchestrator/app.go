...
func (a *Orchestrator) Init() error {
	...
	if err := a.writeManagedSites(); err != nil {
		return err
	}
...
func (a *Orchestrator) writeManagedSites() error {
	containers, err := managedInstances()
	if err != nil {
		return err
	}
	for _, container := range containers {
		...
		if err := ValidateName(name); err != nil {
			return fmt.Errorf("invalid managed instance name for %s: %w", container, err)
		}
		if err := a.writeSite(name); err != nil {
			return err
		}
...
`, name, instanceContainer(name), name, instanceContainer(name))
